...
public class Utils {
    ...
    public static String from(String fromText) {
        return convert(fromText, dark_gray + "[" + gold + "CoolIdeas" + dark_gray + "] " + reset);
    }
    ...
    public static float calculateScaledBonus(PlayerSouls playerSouls, Player attacker) {
        //scale percent from 0 - 0.5 depending on xp level 0 - 30
        return 1f + ((((float) playerSouls.getSouls() / (float) playerSouls.getSoulCapacity(attacker)) / 2f) * (attacker.experienceLevel / 30f));
    }
